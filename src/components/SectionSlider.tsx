...
// Heading Component
const SliderHeading: React.FC<HeadingProps> = ({
  heading,
  subHeading,
  dimHeading,
  level = "h2",
...
const SectionSlider: React.FC<SectionSliderProps> = ({
  ...
  const { autoSlide, autoSlideInterval, showButtons, loop, slidesToScroll } =
    mergedConfig;
  ...
  const translateX = isInfiniteMode
    ? 0
  ...
  const categoriesToRender =
    isInfiniteMode && autoSlide && loop ? duplicatedCategories : categories;
    ...
    <div className={clsx("section-slider-new-categories relative", className)}>
      ...
      >
        ...
        <div
          ...
          {...(({ ref, ...handlers }) => handlers)(swipeHandlers)}
        >
          <motion.div
            animate={controls}
            className="flex"
            style={{
              transform: isInfiniteMode
                ? undefined
                : `translateX(${translateX}%)`,
              width: isInfiniteMode
                ? "auto"
                : `${(totalSlides / cardsPerView) * 100}%`,
            }}
            transition={{
