...
const PostCardLikeBtn: FC<Props> = ({
  className,
  likeCount = 0,
  liked = false,
  color,
  ...
  useEffect(() => {
    if (userFavourites && post?._id) {
      const isPostLiked = userFavourites.some(
        (fav: Favorite) => fav.postId?._id === post._id
      );
  ...
  const handleLikeClick = async () => {
    ...
    try {
      ...
      setOptimisticLikeCount((prev) => (newLikedState ? prev - 1 : prev + 1));
      toast.error(
        `Failed to ${newLikedState ? "add to" : "remove from"} favorites`
      );
    }
  };
