...
} from "@/components/tiptap-extension/single-heading-document";
  ...
  const featuredImageEditor = useEditor({
    ...
    editorProps: {
      attributes: {
        ...
        class:
          "prose mx-auto max-w-screen-md dark:prose-invert lg:prose-lg featuredImageEditor",
      },
    },
    extensions: [
      SingleImageDocument,
  ...
  const titleEditor = useEditor({
    ...
    editorProps: {
      attributes: {
        ...
        class:
          "prose mx-auto max-w-screen-md dark:prose-invert lg:prose-lg titleEditor",
      },
    ...
    onUpdate: ({ editor }) => {
      ...
      if (content.content && content.content.length > 1) {
        editor.commands.setContent(
          "<h1>" + content?.content?.[0]?.content?.[0]?.text + "</h1>"
        );
      }
    },
    content: "<h1></h1>",
    ...
    <EditorContext.Provider value={{ editor }}>
      <>
        <div className="sticky top-0 z-10 flex h-[var(--tt-toolbar-height)] items-center gap-2 border-b border-neutral-200 bg-[var(--tt-toolbar-bg-color)] px-2 dark:border-neutral-800">
          <div className="flex items-center gap-1">
            ...
            <span className="me-2 hidden font-light text-neutral-500 sm:block dark:text-neutral-400">
              /
            </span>
      ...
      >
        ...
        <DialogBody>
          <div className="flex flex-col gap-5 text-sm/6">
            ...
            <div className="flex flex-wrap gap-2.5">
              ...
              <div>
                {selectedTags.map((tag) => tag.name).join(", ") || "No tags"}
              </div>
            </div>
            <Divider />
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <div className="line-clamp-1">
                {featuredImageUrl || "No featured image"}
              </div>
            </div>
            <Divider />
        ...
        <DialogActions>
          ...
          <SharedButton onClick={() => setIsOpenPreview(false)}>
            Publish
          </SharedButton>
        </DialogActions>
      </Dialog>
