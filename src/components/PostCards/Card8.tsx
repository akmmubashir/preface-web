...
const Card8: FC<Props> = ({
  className,
  post,
  ratio = "aspect-3/4 sm:aspect-2/1",
  lang,
}) => {
  const {
    title,
  ...
  return (
    <div
      className={clsx(
        "group post-card-8 relative overflow-hidden rounded-3xl",
        className
      )}
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col p-4 sm:p-6">
        ...
        <h2
          className="mt-4 !line-clamp-2 block text-lg font-semibold text-white sm:text-xl"
          title={title}
        >
          {title}
        </h2>
