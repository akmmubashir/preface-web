...
export const categoryKeys = {
  ...
  quranSubcategories: () =>
    [...categoryKeys.all, "quran", "subcategories"] as const,
};
...
export const useTrendingCategories = (limit?: number) => {
  return useQuery({
    ...
    queryFn: async (): Promise<CategoriesResponse> => {
      const response = await clientApi.get<Category[]>(
        "/api/frontend/trending-topics",
        {
...
export const useQuranSubcategories = (limit?: number) => {
  return useQuery({
    ...
    queryFn: async (): Promise<CategoriesResponse> => {
      const response = await clientApi.get<Category[]>(
        "/api/frontend/quran-subcategories",
        {
...
export const useCategory = (categoryId: string) => {
  return useQuery({
    ...
    queryFn: async (): Promise<CategoryResponse> => {
      const response = await clientApi.get<Category>(
        `/api/frontend/categories/${categoryId}`
      );
      return response;
    },
    enabled: !!categoryId,
...
export const useUpdateCategory = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await clientApi.put<Category>(
