...
export const usePostsByCategory = (categoryId: string, limit?: number) => {
  return useQuery({
    ...
    queryFn: async (): Promise<PostsResponse> => {
      const response = await clientApi.get<Post[]>(
        `/api/frontend/categories/${categoryId}/posts`,
        {
          params: { limit: limit || 10 },
        }
      );
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await clientApi.post(
        `/api/frontend/posts/${postId}/${endpoint}`,
        {}
      );
      return response;
    },
        ...
        (old: PostResponse | undefined) => {
          ...
          return {
            ...
            data: {
              ...
              isLiked: action === "like",
              likeCount:
                (old.data.likeCount || 0) + (action === "like" ? 1 : -1),
            },
          };
        }
      );
...
export const useIncrementViewCount = () => {
  return useMutation({
    mutationFn: async (postId: string) => {
      const response = await clientApi.post(
        `/api/frontend/posts/${postId}/view`,
        {}
      );
      return response;
    },
