...
export async function generateMetadata({
  ...
  if (
    !article ||
    article.includes(".") ||
    article.includes("com.chrome.devtools.json")
  ) {
    return {
...
const Page = async ({
  ...
  const filteredSubcategoryPosts =
    subcategoryPosts?.list?.filter(
      (post) => post.postType?.name === "Article"
    ) || [];
  ...
  const subcategoryList = await serverFetch.get(
