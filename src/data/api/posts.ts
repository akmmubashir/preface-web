...
export const getLatestArticles = async (lang?: string) => {
  try {
    const data = await serverFetch.get<{ data: any[] }>(
      "/api/frontend/latest-articles",
      {
        language: lang,
...
export const getLatestVideos = async (
  ...
  try {
    const response = await serverFetch.get<{ data: any[] }>(
      "/api/frontend/latest-videos",
      {
        language: lang,
        next: { revalidate: 60 }, // Revalidate every 60 seconds
        limit: options?.limit,
...
export const getIslamForBeginners = async (lang?: string) => {
  try {
    const response = await serverFetch.get<{ data: { latestArticles: any[] } }>(
      "/api/frontend/islam-for-beginners",
      {
        language: lang,
        next: { revalidate: 60 }, // Revalidate every 60 seconds
      }
...
export const getQuranLatestArticles = async (
  lang?: string,
  options?: { limit?: number }
) => {
  try {
    const response = await serverFetch.get<{ data: any[] }>(
      "/api/frontend/quran-latest-articles",
      {
        language: lang,
        next: { revalidate: 60 }, // Revalidate every 60 seconds
        limit: options?.limit,
...
export const getSubcategoryPosts = async (
  ...
  try {
    ...
    console.error(
      `Failed to fetch posts for subcategory: ${categorySlug}`,
      error
    );
    return {
      categories: [],
...
export const getPostBySlug = async (slug: string) => {
  try {
    ...
    if (
      !slug ||
      slug.includes(".") ||
      slug.includes("com.chrome.devtools.json")
    ) {
...
export const getTopicsWithArticles = async (lang?: string) => {
  try {
    const response = await serverFetch.get<{ data: any[] }>(
      "/api/frontend/topics-with-articles",
      {
        language: lang,
        next: { revalidate: 60 }, // Revalidate every 60 seconds
      }
...
export const getCategoryBySlug = async (category: string, lang?: string) => {
  try {
    const response = await serverFetch.get<{ data: any }>(
      `/api/frontend/category/slug/${category}`,
      {
        language: lang,
        next: { revalidate: 60 }, // Revalidate every 60 seconds
      }
